## ⚠️ ВАЖНО: Миграции нужно выполнять в порядке!

### Порядок выполнения миграций:

1. **Исправление ENUM и profiles** - ⚠️ **ОБЯЗАТЕЛЬНО ПЕРВЫМ!**
2. **Изделия (products)** - ⚠️ **ВТОРЫМ!**
3. Контроль качества (quality_control)
4. Материалы (materials)
5. Календарь (calendar_events)

⚠️ **ВАЖНО:** Миграция `supabase_migrations_fix_enums.sql` должна быть выполнена ПЕРЕД всеми остальными, так как она создаёт таблицу `profiles`, на которую ссылаются другие таблицы!

---

## Исправление ENUM и profiles ⚠️ ВЫПОЛНИТЬ ПЕРВЫМ!

1) Откройте Supabase → SQL Editor → New query
2) Вставьте полностью `supabase_migrations_fix_enums.sql`
3) Run
4) Проверка:
   - Table: public.profiles
   - ENUM: task_status содержит 'review', project_status содержит 'archived'
   - Foreign key: task_comments.author_id ссылается на profiles.id

---

## Изделия / Продукты (products) ⚠️ ВЫПОЛНИТЬ ВТОРЫМ!

1) Откройте Supabase → SQL Editor → New query
2) Вставьте полностью `supabase_migrations_products.sql`
3) Run
4) Проверка:
   - Table: public.products
   - Columns: id, name, sku, description, status, progress, assignee_id, deadline, unit_price, quantity_in_stock, created_at, updated_at
   - RLS: read/insert/update/delete policies для всех авторизованных пользователей
   - Realtime: таблица в публикации supabase_realtime
   - Демо-данные: 5 изделий (Шкаф-купе, Кухонный гарнитур, Прихожая, Стол, Кровать)

⚠️ **Эта миграция должна быть выполнена ПЕРЕД materials, т.к. product_materials ссылается на products!**
...
4) Проверка:
   ...
   - Демо-данные: **120+ материалов**, включая:
     * 20 декоров ЛДСП EGGER (Дуб Галифакс, Кашемир, Хромикс и др.)
     * 7 кромок EGGER (ABS 2мм, ПВХ 0.4мм/1мм)
     * Фурнитура Blum (11 позиций: петли Clip Top, Tandem, Movento, Aventos)
     * Фурнитура Hettich (8 позиций: Sensys, InnoTech, Quadro V6+, AvanTech)
     * Фурнитура GTV (6 позиций, бюджетная)
     * Ручки мебельные (6 видов)
     * Крепёж (16 позиций: конфирматы, минификсы, шканты, полкодержатели)
     * Освещение (6 позиций: LED ленты, профили, датчики)
     * Системы хранения (7 позиций: штанги, корзины, пантографы)
     * Покрытия и клеи (7 позиций)
     * Упаковка (5 позиций)
